...
namespace TheatreDAL
{
    public class RepresentationDAO
    {
        ...
        public static bool SupprimerRepresentation(int id)
        {
            ...
            {
                if (ex.Number == 547) // En cas de contrainte de clé étrangère
                {
        ...
        public static List<RepresentationVue> SearchRepresentation(string nomTheatre)
        {
            int id, nbPlaceMax;
            string lieu, heure, tarif, date, theatre;
        ...
        public static List<RepresentationVue> SearchRepresentation(string nomTheatre, string date1, string date2)
        {
            int id, nbPlaceMax;
            string lieu, heure, tarif, date, theatre;
            ...
            string query = @"
                ...
                LEFT JOIN Pieces p ON r.rep_pie = p.pie_id 
                LEFT JOIN Tarif t ON r.rep_tar = t.tar_id
                WHERE rep_date BETWEEN @date1 AND @date2";
            connection.Open();
            if (nomTheatre != "")
            {
                query += " AND p.pie_nom = @nom;";
            }
            else
            {
                query += ";";
